...
public class BumpmapMaterial extends AAdvancedMaterial {
		protected static final String mFShader = 
		    ...
		    "	vec3 L = vec3(0.0);\n" +
		    "%LIGHT_CODE%" +
			" 	vec3 color = intensity * texture2D(" + UNI_DIFFUSE_TEX + ", vTextureCoord).rgb;" +
		    "	gl_FragColor = vec4(color, 1.0) + " + UNI_AMBIENT_COLOR + " * " + UNI_AMBIENT_INTENSITY + ";\n" + 
	...
	public void setShaders(String vertexShader, String fragmentShader) {
		...
		for(int i=0; i<mLights.size(); ++i) {
			...
			sb.append("intensity += ").append(UNI_LIGHT_POWER).append(i).append(" * max(dot(bumpnormal, L), 0.1) * vAttenuation").append(i).append(";\n");
		}
